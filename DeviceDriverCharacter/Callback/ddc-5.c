...
static int driver_openi ( struct inode *inode, struct file * file )
{
...
static ssize_t driver_read ( struct file *filp, char __user *buf, size_t len, loff_t * off ){
	...
	printk(KERN_INFO "Longitud: %ld, offst%lld \n",len, *off);
	if( *off == 0 && len > 0){
		strcpy( buffer,"Hola!!!");
		lon = strlen(buffer) + 1 ;
		ret = copy_to_user(buf,buffer,lon);
		if(ret){
			printk(KERN_ERR "Error al copiar el buffer a espacio de usaurio");
			return -EFAULT;
		}
		(*off) += lon;
		return lon;
...
static ssize_t driver_write( struct file *filp, const char *buf, size_t len, loff_t * off )
{
	...
	printk(KERN_INFO "Len: %d, offset %lld  \n",len, off);
